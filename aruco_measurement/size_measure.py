...
if corners:
    ...
    for cnt in contours:
        ...
        cv2.putText(img, "Length: {} cm".format(round(object_width, 1)), (int(
            x - 100), int(y - 20)), cv2.FONT_HERSHEY_PLAIN, 2, (100, 200, 0), 2)
        cv2.putText(img, "Width: {} cm".format(round(object_height, 1)), (int(
            x - 100), int(y + 15)), cv2.FONT_HERSHEY_PLAIN, 2, (100, 200, 0), 2)
        print(object_width, object_height)

cv2.imshow("Fish Size Measurement", img)
key = cv2.waitKey(0)
