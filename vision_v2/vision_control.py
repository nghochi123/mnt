import numpy as np
import cv2
from aruco_with_undistorted_modified import Aruco as aruco

from constants.vars import VIDEO_LINK, HOUGH_MAX, HOUGH_MIN, HOUGH_PARAM_1, HOUGH_PARAM_2


def resize(img):
    return cv2.resize(img, (960, 540)) #original 1024 , 576

def reduce_size(img):
    return cv2.resize(img, (480, 270))
...
class VisionController():
    ...
    def circle_callibration(self):
        while True:
            ...
            cv2.imshow('Callibration', frame) #add back the resize frame
    ...
    def background_callibration(self):
        while(True):
            ...
            if cv2.waitKey(1) & 0xFF == ord('q'):
                break
            '''
            if cv2.waitKey(1) & 0xFF == ord('w'):  #let user restart background callibration
                cv2.destroyAllWindows()
                continue
            '''
        cv2.destroyAllWindows()
